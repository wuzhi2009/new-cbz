...
class Historical extends Component {
    state = { con:"", total: 0, page:1, pageSize:10, data:[] } 
    componentDidUpdate(oldProps) {
        ...
        if (searchValue && searchValue !== oldProps.searchValue) {
            ...
            get(`/monitoring/historicalList?con=${con}&page=1&pageSize=${pageSize}`).then(res => {
                if (res.data.code === 200) {
                    this.setState({total: res.data.total, data: res.data.data});
                }
            })
    ...
    componentDidMount() {
        get(`/monitoring/historicalList`).then(res => {
            if (res.data.code === 200) {
                this.setState({data: res.data.data, page: 1, total: res.data.total});
            }
        })
    }
    render() { 
        const { navigate } = this.props.router;
        const { page, total, data } = this.state;
        return (
            <>
                ...
                <div style={ {position: 'relative', top: 55, margin: '0 20px'} }>
                    ...
                    <div style={ {marginTop: 15} }>
                        <Pagination defaultCurrent={page} total={total} onChange={(page, pageSize) => {
                            ...
                            get(`/monitoring/historicalList?con=${con}&page=${page}&pageSize=${pageSize}`).then(res => {
                                if (res.data.code === 200) {
                                    this.setState({total: res.data.total, data: res.data.data});
                                }
                            })
